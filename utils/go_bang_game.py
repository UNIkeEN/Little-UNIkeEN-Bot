...
def negative_direction(direction: GoBangDirection):
...
class GoBangGame():
    ...
    def __init__(self):
        ...
        self.buffer = [[[[GoBangPiece.NOTHING, 0, 0] for _ in range(8)] for _ in range(self.COLS)] for _ in
                       range(self.ROWS)]
        self.pieceOrder = []
    ...
    def _check_five(self, piece: GoBangPiece, pos: Tuple) -> bool:
        """判断是否为 `五子连珠`"""
        bufferAtPos = self.buffer[pos[0]][pos[1]]
        for direction in range(4):
            anotherDirection = negative_direction(direction)
            dist = bufferAtPos[direction][piece] + bufferAtPos[direction][piece]
            if dist == 4:
                return True
        return False

    def _check_chang_lian(self, piece: GoBangPiece, pos: Tuple) -> bool:
        """判断是否为 `长连`"""
        bufferAtPos = self.buffer[pos[0]][pos[1]]
        for direction in range(4):
            anotherDirection = negative_direction(direction)
    ...
    def _check_mian(self, piece: GoBangPiece, pos: Tuple) -> bool:
        """判断是否为 `眠n`"""

    def _check_double3(self, piece: GoBangPiece, pos: Tuple) -> bool:
        """判断是否为 `双三`"""
        bufferAtPos = self.buffer[pos[0]][pos[1]]
        for dir1 in range(4):
            dir2 = negative_direction(dir1)
            # TODO:
        return False

    def check_forbid(self, piece: GoBangPiece, pos: Tuple) -> bool:
        """判断是否为 `禁手`"""
        if self._check_five(piece, pos): return False
        if self._check_chang_lian(piece, pos): return True
        if self._check_double3(piece, pos): return True
        return False

    def update_buffer(self, pos: Tuple, piece: GoBangPiece):
        """更新缓存"""
        bufferAtPos = self.buffer[pos[0]][pos[1]]
        for direction in range(8):
            ...
            targetLen = dist + bufferAtPos[negative_direction(direction)][piece]
            targetBuffer[negative_direction(direction)][piece] = targetLen
            if targetLen == 5:
                self.doneFlag = True
...
def draw_go_bang_pic(black, white, groupId=''):
    ...
    # 星
    def draw_piece(i, j, radius=checkerboardDeltaRow // 2 - 5, color=COLOR_BLACK):
        # 下标从1开始
        i = NROWS - i
        j = j - 1
    ...
    # 星
    draw_piece((NROWS + 1) // 2, (NCOLS + 1) // 2, 4)
    draw_piece(4, 4, 4)
    draw_piece(4, NCOLS + 1 - 4, 4)
    draw_piece(NROWS + 1 - 4, 4, 4)
    draw_piece(NROWS + 1 - 4, NCOLS + 1 - 4, 4)
    # 棋子
    for p in black:
        draw_piece(p[0] + 1, p[1] + 1, color=COLOR_BLACK)
    for p in white:
        draw_piece(p[0] + 1, p[1] + 1, color=COLOR_WHITE)
