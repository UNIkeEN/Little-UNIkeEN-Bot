...
class GetUniAgenda(StandardPlugin): 
    ...
    def executeEvent(self, msg:str, data:Any) -> Union[None, str]:
        ...
        succ, info = makeAgenda(qq_id=data['user_id'])
        if not succ:
...
def makeAgenda(qq_id:int)->Tuple[bool, str]:
    """
    制作uag日程列表
    @qq_id: 用户qq号
    ...
    imgSavePath = drawAgenda(events=events, qq_id=qq_id)
    if imgSavePath == None:
        return False, '图片绘制失败'
    return True, imgSavePath

def parseSummary(courseName:str)->Optional[Dict]:
    """
    Canvas日程名称分词
    @courseName: 原日程名称
...
def syncCanvas(qq_id:int)->Optional[List]:
    """
    同步canvas并生成uag日程项
    @qq_id: 用户qq号
...
def drawAgenda(events:List[Tuple], qq_id:int)->str:
    """绘制并保存日历图像，返回图像存储位置"""
    try:
        ...
        tmp = ResponseImage()
        img = tmp.drawRoundedRectangle(30, 30, width-30, height-30, PALETTE_WHITE, border = True, borderWidth=3,target=img)
        ...
        for i in range(-3,4):
            ...
            draw.text((x,y_),TEXT_WEEK_LIST[d.weekday()], (PALETTE_BLACK if i!=0 else PALETTE_SJTU_DARKRED), FONT_SYHT_M24)
            if i==0:
        ...
        img = tmp.drawRoundedRectangle(x-txt_size[0]/2-10, height-120, x+txt_size[0]/2+10, height-100+txt_size[1], PALETTE_SJTU_DARKRED,target=img)
        ...
        for title,subtitle, tbegin, tend, clrborder, clrback, icon in events:
            ...
            img = tmp.drawRoundedRectangle(x1, h_top, x2, h_top+120, clrback, border=True, borderColor=clrborder, borderWidth=2, target=img)
            
            icon_width=0
            if icon!=None:
                icon = tmp.openImage(icon)
                if icon.height>118:
