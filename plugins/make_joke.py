...
def make_resource(what: str):
    today = datetime.date.today()
    tomorrow = today + datetime.timedelta(days=1)
    return {
        "app": "com.tencent.miniapp", "desc": "", "view": "notification", "ver": "0.0.0.1", "prompt": "该群即将解散",
        "meta": {
            "notification": {
                "appInfo": {
                    "appName": "腾讯官方通知", "appType": 4, "appid": 2034149631,
                    "iconUrl": "https://q.qlogo.cn/headimg_dl?dst_uin=D00D2B5BF35B8B79453E11B5CAD17370&spec=100"
                }, "data": [{
                    "title": "通知内容", "value": "本群因%s，将于15分钟后解散，请自行遵守相关规定。\n\n\n腾讯科技\n%s" % (
                    what, tomorrow.strftime('%Y年%m月%d日'))
                }], "emphasis_keyword": ""
            }
        }
    }
...
class MakeJoke(StandardPlugin):
    ...
    def execute_event(self, msg, data):
        ...
        if jokeWhat == '':
            jokeWhat = '你太美'
        send(target, '[CQ:json,data=%s]' % gocq_quote(json.dumps(make_resource(jokeWhat))), data['message_type'])
        return 'OK'
