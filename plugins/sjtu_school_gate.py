...
def get_school_gate_status() -> Optional[List[Dict]]:
...
def draw_school_gate_pic(gatesInfo: List[Dict], savePath: str) -> Tuple[bool, str]:
    ...
    for gateInfo in gatesInfo:
        ...
        gateCard.add_card(ResponseImage.RichContentCard(
            raw_content=content,
            titleFontColor=PALETTE_CYAN,
        ))
    gateCard.generate_image(savePath)
...
class SjtuSchoolGate(StandardPlugin):
    ...
    def execute_event(self, msg: str, data: Any) -> Union[None, str]:
        ...
        gatesInfo = get_school_gate_status()
        if gatesInfo == None:
            ...
            succ, result = draw_school_gate_pic(gatesInfo, savePath)
            if succ:
