from utils.basic_configs import ROOT_PATH, SAVE_TMP_PATH, sqlConfig, BOT_SELF_QQ
from utils.basic_event import send, warning, startswith_in, get_avatar_pic, get_group_avatar_pic
from typing import Union, Tuple, Any, List, Optional
from utils.standard_plugin import StandardPlugin
from PIL import Image, ImageDraw, ImageFont
from utils.sql_utils import new_sql_session, mysql
...
class ActReportPlugin(StandardPlugin):
    ...
    def execute_event(self, msg: str, data: Any) -> Union[None, str]:
        imgPath = get_my_activity(data['user_id'], data['group_id'])
        if imgPath == None:
...
class ActRankPlugin(StandardPlugin):
    ...
    def execute_event(self, msg: str, data: Any) -> Union[None, str]:
        imgPath = get_group_activity_rank(data['group_id'])
        if imgPath == None:
            send(data['group_id'], '[CQ:reply,id=%d]生成失败' % data['message_id'], data['message_type'])
        else:
            imgPath = imgPath if os.path.isabs(imgPath) else os.path.join(ROOT_PATH, imgPath)
...
def get_my_activity(user_id: int, group_id: int) -> Optional[str]:
    """生成并绘制水群报告
    @user_id: 用户QQ
    @group_id: 群
    ...
    try:
        ...
        mycursor.execute(
            "SELECT message_seq from `clearChatLog` where user_id = %d and group_id = %d" % (user_id, group_id))
        result = list(mycursor)
        minSeq = None if len(result) == 0 else result[0][0]
        if minSeq == None:
            mycursor.execute(
                "SELECT time, message FROM messageRecord where user_id=%d and group_id=%d" % (user_id, group_id))
        else:
            mycursor.execute(
                "SELECT time, message FROM messageRecord where user_id=%d and group_id=%d and message_seq > %d" % (
                user_id, group_id, minSeq))
        ...
        ActCards.generate_image(save_path)
        return save_path
...
def get_group_activity_rank(group_id: int) -> Optional[str]:
    """生成并绘制水群排行
    @group_id: 群号
    ...
    try:
        ...
        ActRankCards.generate_image(save_path)
        return save_path
    except mysql.connector.Error as e:
