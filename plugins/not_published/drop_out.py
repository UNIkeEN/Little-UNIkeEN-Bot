...
class DropOut(StandardPlugin):
    ...
    def execute_event(self, msg: str, data: Any) -> Union[None, str]:
        ...
        picPath = draw_dropout(name, data['user_id'], dropoutDate)
...
def gen_chinese_date(d: date) -> str:
    chineseDigitDict = {
        '1': '一', '2': '二', '3': '三',
        '4': '四', '5': '五', '6': '六',
        '7': '七', '8': '八', '9': '九', '0': '〇'
...
def draw_dropout(name: str, qq_id: int, d: date = date(2022, 9, 8)) -> str:
    ...
    paste_line(img, 567, 1077, name + ' 同学:')
    paste_line(img, 677, 1215, '你已被我校 工科试验班类 专业退学！')
    paste_line(img, 677, 1363, '请于%s前凭本通知书办理离校手续。' % gen_chinese_date(d))
