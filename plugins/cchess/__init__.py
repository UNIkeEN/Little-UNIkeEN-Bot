...
from .chessdb import draw_chessdb
...
class ChineseChessPlugin(StandardPlugin):
        ...
        if msg in self.startCommands:
            if self.game_running(data):
                ...
                if msg in ['下象棋', '-lxq']:
                    ...
                    send(target, '群友 {} 执红，向大家发起中国象棋挑战，输入“应战”接受挑战吧！'.format(player),
                         data['message_type'])
                else:
                    game.player_black = player
                    send(target, '群友 {} 执黑，向大家发起中国象棋挑战，输入“应战”接受挑战吧！'.format(player),
                         data['message_type'])
            ...
            if game == None:
                ...
                if game.player_red == None:
                    if game.player_black.id == player.id:
                        ...
                        send(target,
                             f'[CQ:image,file=files:///{imgPath}]对局开始，请执红者 [CQ:at,qq={game.player_red.id}] 先行',
                             data['message_type'])
                else:
                ...
                send(target, '[CQ:reply,id={}]内部错误 game.player_red == None and game.player_black == None'.format(
                    data['message_id']), data['message_type'])
            ...
            if game == None:
                ...
                if move == None:
                    send(target, "[CQ:reply,id={}]请发送正确的走法，如 “炮二平五” 或 “h2e2”".format(data['message_id']),
                         data['message_type'])
                else:
                    ...
                    if result == MoveResult.ILLEAGAL:
                        ...
                        imgPath = self.draw_board(game, data)
                        send(target, f'[CQ:image,file=files:///{imgPath}]黑方被将死，恭喜 {game.player_red} 获胜！',
                             data['message_type'])
                        self.stop_game(data)
                    elif result == MoveResult.BLACK_WIN:
                        imgPath = self.draw_board(game, data)
                        send(target, f'[CQ:image,file=files:///{imgPath}]红方被将死，恭喜 {game.player_black} 获胜！',
                             data['message_type'])
                        self.stop_game(data)
    ...
    def draw_chessdb(self, game, data) -> Optional[str]:
        ...
        result = draw_chessdb(game, imgPath)
        if result:
            return imgPath
        else:
            return None
