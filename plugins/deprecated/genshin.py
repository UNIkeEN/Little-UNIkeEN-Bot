...
class GenshinDailyNote(StandardPlugin):
    ...
    def execute_event(self, msg: str, data: Any) -> Union[None, str]:
        ret = get_genshin_dailynote(data['user_id'])
...
def get_genshin_dailynote(qq_id):
    qq_id = str(qq_id)
    ...
    if data['retcode'] != 0:
        ...
        # 树脂
        draw.text((60, 280), f"原粹树脂：{data['data']['current_resin']}/{data['data']['max_resin']}",
                  fill=(0, 0, 0, 255), font=font_hywh_85w)
        if data['data']['current_resin'] == 160:
            ...
            recover_time = datetime.datetime.now() + datetime.timedelta(
                seconds=int(data['data']['resin_recovery_time']))
            recover_time_day = '今天' if recover_time.day == datetime.datetime.now().day else '明天'
            draw.text((520, 280), f'将于{recover_time_day}{recover_time.strftime("%H:%M")}回满',
                      fill=(175, 175, 175, 255), font=font_hywh_85w)
        # 宝钱
        draw.text((60, 360), f"洞天财瓮：{data['data']['current_home_coin']}/{data['data']['max_home_coin']}",
                  fill=(0, 0, 0, 255), font=font_hywh_85w)
        if data['data']['current_home_coin'] == data['data']['max_home_coin']:
            draw.text((520, 360), f"洞天宝钱已满", fill=(175, 175, 175, 255), font=font_hywh_85w)
        else:
            recover_time = datetime.datetime.now() + datetime.timedelta(
                seconds=int(data['data']['home_coin_recovery_time']))
            d = int(data['data']['home_coin_recovery_time']) // 86400
            if d == 0:
                draw.text((520, 360), f'将于今天{recover_time.strftime("%H:%M")}回满', fill=(175, 175, 175, 255),
                          font=font_hywh_85w)
            elif d == 1:
                draw.text((520, 360), f'将于明天{recover_time.strftime("%H:%M")}回满', fill=(175, 175, 175, 255),
                          font=font_hywh_85w)
            elif d == 2:
                draw.text((520, 360), f'将于后天{recover_time.strftime("%H:%M")}回满', fill=(175, 175, 175, 255),
                          font=font_hywh_85w)
            else:
                draw.text((520, 360), f'将于{d}天后回满', fill=(175, 175, 175, 255), font=font_hywh_85w)
        # 委托
        draw.text((60, 440), f"已完成每日委托：{data['data']['finished_task_num']}/{data['data']['total_task_num']}",
                  fill=(0, 0, 0, 255), font=font_hywh_85w)
        # 周本
        draw.text((60, 520),
                  f"剩余周本消耗减半次数：{data['data']['remain_resin_discount_num']}/{data['data']['resin_discount_num_limit']}",
                  fill=(0, 0, 0, 255), font=font_hywh_85w)
        # 探索派遣
        draw.text((60, 600), f"探索派遣：{data['data']['current_expedition_num']}/{data['data']['max_expedition_num']}",
                  fill=(0, 0, 0, 255), font=font_hywh_85w)
        i = 0
        for record in data['data']['expeditions']:
            ...
            if record['status'] == "Finished":
                ...
                draw.text((60 + i * 177 - int(bool(h)) * 30, 790), f"余{'' if h == 0 else str(h)}时{m}分",
                          fill=(0, 0, 0, 255), font=font_hywh_85w_ms)
