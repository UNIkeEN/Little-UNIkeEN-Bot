    ...
    def execute_event(self, msg: str, data: Any) -> Union[None, str]:
        ...
        if self.patterns['show'].match(msg) != None:
            ...
            if False:
                ...
                imgPath = draw_question_card(questions, group_id)
...
def load_questions(group_id: int) -> dict:
    """获取问题列表
    @group_id: 群号
    @return: 问题列表
    """
...
def dump_questions(group_id: int, questions: dict):
    """存储问题列表
    @group_id: 群号
    @questions: 问题列表
    """
...
def get_answer(group_id, key) -> Tuple[bool, str]:
    answers = load_questions(group_id)
...
def add_question(group_id, key, ans) -> bool:
    ...
    if key not in answers.keys():
        answers[key] = ans
        dump_questions(group_id, answers)
        return True
    else:
        return False
...
def draw_question_card(questions: List[str], group_id: int) -> str:
    """绘制问答列表图像
    @questions: 问题列表
    """
