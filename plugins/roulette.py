...
class _roulette():
    ...
    def get_cmd(self, id, msg):
        ...
        if initPattern.match(msg) != None:
            ...
            if self.status != 'prepare':
                ...
                self.round_index = 0
                return (
                    f"{self.player[0]}和{self.player[1]}的决斗拉开帷幕！\n------\n发送“咔/嘭/嘣/砰/开枪 [开枪次数(可选，默认为1)]”以开枪\n") + (
    ...
    def result(self, win_qqid, loser_qqid):
        ...
        try:
            ...
            draw.text(((width - txt_size[0]) / 2, 290), "俄罗斯轮盘 - 对决结果", fill=(0, 0, 0, 255),
                      font=font_hywh_85w)
            # 圆形蒙版
            ...
            draw.text((210, 490), f'金币+{wager} -> 当前金币：{get_user_coins(win_qqid)}', fill=(175, 175, 175, 255),
                      font=font_hywh_85w_mms)
            draw.text((210, 570), f'失败者：{loser_qqid}', fill=(0, 191, 48, 255), font=font_hywh_85w)
            draw.text((210, 640), f'金币-{wager} -> 当前金币：{get_user_coins(loser_qqid)}', fill=(175, 175, 175, 255),
                      font=font_hywh_85w_mms)
            draw.text((60, 720),
                      '发起新的决斗：\n装弹/轮盘/决斗 [子弹数] [轮盘总格数] [挑战金额] [@决斗对象(可选)]\n举例：装弹 2 7 100 @xxx',
                      fill=(175, 175, 175, 255), font=font_syht_m)
