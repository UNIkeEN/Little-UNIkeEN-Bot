...
from utils.go_bang_game import NCOLS, NROWS, GoBangGame, draw_go_bang_pic
from enum import IntEnum
import traceback

GOBANG_SPEND_COINS = 0
CMD_GOBANG = ['开始五子棋', '取消五子棋', '接受五子棋', '认输']
CMD_GOBANG_ONGOING = lambda txt: re.match(
    '^([1-9]|0[1-9]|1[0-%d])([A-%s])$' % (NROWS % 10, "_ABCDEFGHIJKLMNOPQRSTUVWXYZ"[NCOLS]), txt) != None
    ...
    def get_cmd(self, msg: str, data) -> Union[None, str]:
        ...
        if self.status == GameStatus.FREE:
            ...
            if msg == '接受五子棋':
                ...
                self.round_index = 0
                picPath = draw_go_bang_pic([], [], groupId)
            ...
            if CMD_GOBANG_ONGOING(msg):
                x, y = \
                re.findall('^([1-9]|0[1-9]|1[0-%d])([A-%s])$' % (NROWS % 10, "_ABCDEFGHIJKLMNOPQRSTUVWXYZ"[NCOLS]),
                           msg)[0]
                x = int(x) - 1
                y = ord(y) - ord('A')
                ...
                self.round_index ^= 1
                black, white = self.game.get_piece_locs()
                picPath = draw_go_bang_pic(black, white, groupId)
                picPath = picPath if os.path.isabs(picPath) else os.path.join(ROOT_PATH, picPath)
                self.timer = Timer(60, self.ongoing_timeout)
                self.timer.start()
                return f'[CQ:image,file=files:///{picPath}]'
