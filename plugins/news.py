...
class UpdateNewsAndReport(StandardPlugin, CronStandardPlugin):
    ...
    def tick(self) -> None:
        ...
        if today_pic_str != None:
            for group_id in get_plugin_enabled_groups('newsreport'):
...
def draw_news_card(text: str) -> str:
    ...
    for one_line in txt_parse:
        ...
        i += 1
    draw.text((30, height - 78), '来源:澎湃、人民日报、腾讯新闻、网易新闻、新华网、中国新闻网', fill=(175, 175, 175, 255),
              font=font_syht_m)
    draw.text((30, height - 48), 'A plugin by 北极づ莜蓝, made for Little-UNIkeEN-Bot', fill=(175, 175, 175, 255),
              font=font_syht_m)
