from utils.standard_plugin import StandardPlugin, CronStandardPlugin, GuildStandardPlugin
...
def draw_sjtu_news() -> str:
    ...
    for news in sorted(get_sjtu_news(), key=lambda x: x['time'], reverse=True):
        if news['source'] == None:
            keyword = news['time']
        else:
    ...
    def tick(self, ):
        ...
        for j in get_jwc():
            if j['link'] not in url_list:
                ...
                send_guild_channel_msg(MAIN_GUILD['guild_id'], MAIN_GUILD['channels']['jwc'],
                                       '[CQ:image,file=files:///%s]' % pic)
                # time.sleep(3)
...
class GetJwc(StandardPlugin):
    ...
    def execute_event(self, msg: str, data: Any) -> Union[None, str]:
        ...
        jwc = sorted(get_jwc(), key=lambda x: '%s-%s-%s' % (x['year'], x['month'], x['day']), reverse=True)
        jwcStr = ""
        idx = 1
        for j in jwc[:7]:
...
class SubscribeJwc(StandardPlugin):
    ...
    @staticmethod
    def get_jwc_subscribers() -> Set[int]:
...
def draw_notice_pic(notice) -> str:
    width = 720
    txt_line = ""
