...
from .client_instance import query_announcement
from .mua_api import verify_mua_token


def create_mua_token_sql():
...
def get_all_mua_token(userId: int) -> Dict[str, str]:
    mydb, mycursor = new_sql_session()
...
class MuaTokenBinder(StandardPlugin):
    ...
    def execute_event(self, msg: str, data: Any) -> Union[None, str]:
        ...
        if len(tokenDescription) > 20:
            ...
            try:
                succ, verifiedMuaId = verify_mua_token(token)
                if not succ:
                    send(target, '[CQ:reply,id=%d]token验证失败，请检查token' % data['message_id'], data['message_type'])
                    return 'OK'
                if verifiedMuaId != tokenDescription:
                    send(target,
                         '[CQ:reply,id=%d]token绑定失败，用户输入的MUA ID和服务器返回MUA ID不一致' % data['message_id'],
                         data['message_type'])
                    return 'OK'
            except Exception as e:
                print(e)
                send(target,
                     '[CQ:reply,id=%d]与MUA服务器连接失败，无法验证token真实性，请尝试稍后绑定' % data['message_id'],
                     data['message_type'])
                return 'OK'
            try:
                mydb, mycursor = new_sql_session()
...
#           1 使用权
#           2 拥有权
def get_user_mua_id_permission(userId: int, muaId: str) -> int:
    mydb, mycursor = new_sql_session()
...
def make_empower(userId: int, empowerTargetId: int, muaId: str) -> Tuple[bool, str]:
    mydb, mycursor = new_sql_session()
    mycursor.execute("""select `mua_token`, `empowered` from `muaToken` where 
    `user_id` = %s and `token_description` = %s""", (userId, muaId))
    result = list(mycursor)
    if len(result) == 0:
        return False, '您不存在名为“%s”的MUA ID，请输入“-muabind [MUAID] [TOKEN]”绑定之。经查询，您名下的MUA ID有：%s' % (
        muaId, '，'.join(get_all_mua_token(userId).keys()))
    token, empowered = result[0]
    if empowered:
...
class MuaTokenEmpower(StandardPlugin):
    ...
    def execute_event(self, msg: str, data: Any) -> Union[None, str]:
        ...
        if self.triggerPattern1.match(msg) != None:
            ...
            succ, result = make_empower(userId, empowerTarget, muaId)
            ...
            send(target,
                 '[CQ:reply,id=%d]指令解析错误，指令格式为“-muaempower [MUAID] [用户QQ或at用户]”，可以输入“-muaempower TEST 1234”或者“-muaempower TEST @用户1234”' %
...
class MuaTokenUnbinder(StandardPlugin):
    ...
    def execute_event(self, msg: str, data: Any) -> Union[None, str]:
        ...
        try:
            ...
            if list(mycursor)[0][0] == 0:
                send(target,
                     '[CQ:reply,id=%d]您尚未绑定MUAID为 %s 的token，无法解绑' % (data['message_id'], tokenDescription),
                     data['message_type'])
            else:
                mycursor.execute("""
...
class MuaQuery(StandardPlugin):
    ...
    def execute_event(self, msg: str, data: Any) -> Union[None, str]:
        target = data['group_id'] if data['message_type'] == 'group' else data['user_id']
        succ, result = query_announcement(data)
...
class MuaAbstract(StandardPlugin):
    ...
    def execute_event(self, msg: str, data: Any) -> Union[None, str]:
        target = data['group_id'] if data['message_type'] == 'group' else data['user_id']
        succ, result = query_announcement(data, abstract=True)
        send(target, result, data['message_type'])
        return 'OK'
