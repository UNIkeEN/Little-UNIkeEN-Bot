...
from utils.standard_plugin import StandardPlugin, ScheduleStandardPlugin
...
class GenWordCloud(StandardPlugin, ScheduleStandardPlugin):
    ...
    def __init__(self):
        ...
        if GenWordCloud.monitorSemaphore.acquire(blocking=False):
            ...
            _content = [line.strip() for line in
                        open('resources/corpus/wc_stopwords.txt', 'r', encoding='utf-8-sig').readlines()]
    ...
    def tick(self):
        ...
        def gen_send_save_img(group_id):
            wcPic = self.gen_word_cloud(group_id)
            ...
            img.save(picPath)
            if group_id in get_plugin_enabled_groups('wcdaily'):
        ...
        for group_id in APPLY_GROUP_ID:
            thread = Thread(target=gen_send_save_img, args=(group_id,))
            thread.daemon = True
            thread.start()
    ...
    def gen_word_cloud(self, group_id) -> Optional[Image.Image]:
        try:
            mydb, mycursor = new_sql_session(autocommit=False)
            mycursor.execute(
                "SELECT message FROM messageRecord WHERE group_id=%d and TO_DAYS(NOW( ))-TO_DAYS(time)<=1 and user_id!=%d" % (
                group_id, BOT_SELF_QQ))
            ...
            color_list = ['#ec1c24', '#ec1c42', '#ec3b1c', '#e00d0d', '#fe3f3f', '#f59037', '#f57037', '#fb922a',
                          '#fba42a', '#fbbc2a', '#ffa710', '#ffcb10']
