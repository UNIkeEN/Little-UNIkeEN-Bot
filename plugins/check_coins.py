...
class CheckTransactions(StandardPlugin):  # 查询近期交易记录
    ...
    def execute_event(self, msg: str, data: Any) -> Union[None, str]:
        ...
        send(target, '[CQ:reply,id=%d][CQ:image,file=files:///%s]' % (data['message_id'], picPath),
             data['message_type'])
        return "OK"
...
def draw_trans_cards(id: int):
    ...
    for id, timestp, qq, change, balance, description in trans_list:
        ...
        draw.text((width - txt_size[0] - 90, 70 + 110 * i), str(change / 100),
                  fill=(FONT_CLR['r'] if change > 0 else FONT_CLR['g']), font=font_hywh_85w_ms)
        txt_size = draw.textsize(f"余额：{str(balance / 100)}", font=font_hywh_85w_s)
        draw.text((width - txt_size[0] - 90, 70 + 110 * i + 50), f"余额：{str(balance / 100)}",
                  fill=(175, 175, 175, 255), font=font_hywh_85w_s)
        i += 1
