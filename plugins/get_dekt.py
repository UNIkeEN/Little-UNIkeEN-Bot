...
from utils.basic_event import send, warning, init_image_template, draw_rounded_rectangle
from utils.channel_api import send_guild_channel_msg
...
def download_actlist():
    ...
    def parse_cookie(cookieStr: str):
        cookies = []
    ...
    time.sleep(5)
    cookies = parse_cookie(JAC_COOKIE)
    ...
    for entry in result['log']['entries']:
        ...
        if "api/wmt/secondclass/fmGetNewestActivityList" in _url:
            ...
            # 获取接口返回内容
            with open(os.path.join(DEKT_SOURCE_DIR, "dekt-%s.json" % datetime.now().strftime("%Y-%m-%d-%H%M%S")),
                      "w") as f:
                f.write(_content)
    server.stop()
    driver.close()
    ...
    def tick(self) -> None:
        ...
        try:
            ...
            if data_1['data'][0]['id'] != data_2['data'][0]['id']:
                ...
                for group_id in get_plugin_enabled_groups('dekt'):
...
def timestamp_to_time(timestp):
    time_local = time.localtime(float(timestp / 1000))
    dt = time.strftime("%Y-%m-%d %H:%M:%S", time_local)
    return str(dt)


def get_newest_dekt_json():
...
def new_actlist_pic():
    ...
    actlist, rettime = get_newest_dekt_json()
    width = 880
    ...
    for activity in actlist:
        ...
        draw.text((l + 150, h + 80), '活动时间：' + timestamp_to_time(activity['activeStartTime']) + ' ~ ' + timestamp_to_time(
            activity['activeEndTime']), fill=(115, 115, 115, 255), font=font_syhtmed_18)
        try:
            ...
            enroll_time = '报名时间：' + timestamp_to_time(activity['enrollStartTime']) + ' ~ ' + timestamp_to_time(
            ...
            img = draw_rounded_rectangle(img, x1=l + 160 + txt_size[0], y1=h + 110,
                                         x2=l + 180 + txt_size[0] + txt_size_2[0], y2=h + txt_size_2[1] + 130,
                                         fill=BACK_CLR[clr])
        ...
        draw.text((l + 150, h + 160),
                  '招募人数：' + str(activity['recruitQty']) + '  |  组织单位：' + activity['sponsor'][:21],
        ...
        if len(activity['sponsor']) > 20:
            ...
            draw.text((l + 150 + txt_size[0], h + 163 + txt_size[1]), t, fill=(115, 115, 115, 255),
                      font=font_syhtmed_18)
        h += 240
    txt_size = draw.textsize(f'活动列表更新时间 {rettime[:10]} {rettime[11:13]}:{rettime[13:15]}:{rettime[15:]}',
                             font=font_syhtmed_18)
    draw.text((width / 2 - txt_size[0] / 2, height - 85),
              f'活动列表更新时间 {rettime[:10]} {rettime[11:13]}:{rettime[13:15]}:{rettime[15:]}',
              fill=(115, 115, 115, 255), font=font_syhtmed_18)
