...
def get_lib_info():
    ...
    for i in range(len(data)):
        ...
        draw.text((nw_x + 30, nw_y + 60), f"在馆人数 {record['inCounter']}/{record['max']}", fill=FONT_CLR[clr],
                  font=font_hywh_85w_s)
...
def get_canteen_info():
    ...
    for i in range(len(data)):
        ...
        draw.text((nw_x + 30, nw_y + 64), f"已供应量 {record['Seat_u']}/{record['Seat_s']}", fill=FONT_CLR[clr],
                  font=font_hywh_85w_xs)
        txt_per = (str(percent) + "%") if clr != "h" else "关闭"
