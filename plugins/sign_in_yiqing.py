...
FORTUNE_TXT = [['r', "大吉"], ['r', "中吉"], ['r', "小吉"], ['g', "中平"], ['h', "小凶"], ['h', "中凶"], ['h', "大凶"],
               ['r', "奆🐔"], ['h', '奆🐻']]
...
def draw_signinbanner(qq_id, add_coins, now_coins, fortune):
    ...
    # ID
    img = draw_rounded_rectangle(img, l + 180, h, width - 60, h + 150,
                                 (245 + 10 * r // 255, 245 + 10 * g // 255, 245 + 10 * b // 255, 255))
    ...
    # 签到及首签徽章
    if add_coins == -1:
        draw.text((l + 210, h + 85), f"今日已签  |  当前金币：{str(int(now_coins * 100) // 100)}",
                  fill=(255, 128, 64, 255), font=font_syhtmed_28)
    else:
        draw.text((l + 210, h + 85), f"金币+{str(add_coins)}  |  当前金币：{str(int(now_coins * 100) // 100)}",
                  fill=(34, 177, 76, 255), font=font_syhtmed_28)
    h += 180
    # 运势
...
def sign_in(qq_id):
    ...
    if last_sign_date != today_str:
        ...
        try:
            mycursor.execute(
                f"UPDATE `accounts` SET lastSign='{today_str}', fortune='{str(fortune)}' WHERE id='{str(id)}';")
