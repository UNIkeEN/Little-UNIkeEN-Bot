...
class LotteryReminder(ScheduleStandardPlugin):
    ...
    def tick(self) -> None:
        for group_id in get_plugin_enabled_groups('lottery'):
...
class _lottery(ScheduleStandardPlugin):
    ...
    def make_card(self, key_list, win_list):
        BACK_CLR = {'r': (255, 232, 236, 255), 'g': (219, 255, 228, 255), 'h': (234, 234, 234, 255),
                    'o': (254, 232, 199, 255)}
        ...
        for i in range(3):
            draw.rectangle((180 + i * (rec_width + 30), 260, 180 + i * (rec_width + 30) + rec_width, 260 + rec_height),
                           fill=BACK_CLR['o'])
            txt_size = draw.textsize(str(key_list[i]), font=font_hywh_85w_l)
            draw.text((180 + i * (rec_width + 30) + (rec_width - txt_size[0]) / 2, 275), str(key_list[i]),
                      fill=FONT_CLR['o'], font=font_hywh_85w_l)
        for i in range(len_win):
            ...
            for j in range(3):
                ...
                draw.rectangle((180 + j * (rec_width + 30), 260 + 150 * (i + 1), 180 + j * (rec_width + 30) + rec_width,
                                260 + 150 * (i + 1) + rec_height), fill=BACK_CLR[flag])
                txt_size = draw.textsize(str(win_list[i]['num_list'][j]), font=font_hywh_85w_l)
                draw.text((180 + j * (rec_width + 30) + (rec_width - txt_size[0]) / 2, 275 + 150 * (i + 1)),
                          str(win_list[i]['num_list'][j]), fill=FONT_CLR[flag], font=font_hywh_85w_l)
            draw.text((width - 150, 260 + 150 * (i + 1), width, height), f"{TXT_CLASS[win_list[i]['prize']]}等奖",
                      fill=(135, 135, 135, 255), font=font_hywh_85w_s)
            draw.text((width - 150, 260 + 150 * (i + 1) + 40, width, height), f"金币+{PRIZE_NUM[win_list[i]['prize']]}",
                      fill=(135, 135, 135, 255), font=font_hywh_85w_s)
        if len_win == 0:
            txt_size = draw.textsize('本期祈愿，没有人得到了神明的认可', font=font_hywh_85w)
            draw.text(((width - txt_size[0]) / 2, 390), "本期祈愿，没有人得到了神明的认可", fill=(145, 145, 145, 255),
                      font=font_hywh_85w)
