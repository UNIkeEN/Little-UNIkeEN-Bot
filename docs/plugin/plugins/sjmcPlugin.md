...
| ---- | ---- | ---- | ---- | ---- |
| ShowSjmcStatus | StandardPlugin | '-sjmc' | None | 获取交大mc社服务器状态 |
...
![](../../images/plugins/sjmc_status.png) 

## 3. 代码分析

!!! tip "提示：异步获取信息"
    以下代码中采用了异步的信息获取函数 `aio_get_sjmc_info()` ，并行获取多个服务器信息。但仍保留了旧版的 `get_sjmc_info()` 代码。
